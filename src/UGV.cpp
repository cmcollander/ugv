...
public:
	...
	void read() {
		...
		vel[0] = 3.14159265359*ret/3.0;
		vel[1] = device.GetValue(_ABSPEED,2,ret);
		vel[1] = 3.14159265359*ret/3.0;
	...
	void write() {
		...
		//  cmd[x]*159.155*2
		device.SetCommand(_GO,1,(int)(cmd[0]*15.9155*0.33333333));

		device.SetCommand(_GO,2,(int)(cmd[1]*15.9155*0.33333333));
	}
};
